  ...
  async function postSiswa() {
    ...
    if(mode === "Siswa") {
        ...
        const res = await fetch(
          process.env.REACT_APP_API_URL + `classes/${idKelas}/students/add`,
          {
            body: payload,
            method: "POST",
          }
        ...
        payload.append("name", name);
        payload.append("username", cookie);
        payload.append("bobot", bobot);

        const res = await fetch(
            process.env.REACT_APP_API_URL + `classes/${idKelas}/subjects/add`,
            {
              body: payload,
              method: "POST",
            }
...
console.log(data)
  ...
  return (
    <>
      <CustomAppBar kelasId={idKelas} mode="Kelas" position={dataKelas[1]}>
        <Box
          sx={{
            display: "flex",
            flexWrap: "wrap",
